...
do
    create_minmaxtcl=$(sbatch run.sh $prev_npt create_minmaxtcl | cut -f 4 -d' ')
    create_minmax_sbatch=$(sbatch --dependency=afterok:$create_minmaxtcl run.sh $prev_npt minmax_sbatch | cut -f 4 -d' ')
    minmax_submit=$(sbatch --dependency=afterok:$create_minmax_sbatch minmax-npt$prev_npt.sh | cut -f 4 -d' ')

    create_centretcl=$(sbatch --dependency=afterok:$minmax_submit run.sh $prev_npt create_centretcl | cut -f 4 -d' ')
    create_centre_sbatch=$(sbatch --dependency=afterok:$create_centretcl run.sh $prev_npt centre_sbatch | cut -f 4 -d' ')
    centre_submit=$(sbatch --dependency=afterok:$create_centre_sbatch centre-npt$prev_npt.sh | cut -f 4 -d' ')
